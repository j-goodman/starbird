  ...
  if (!this.coordinating) {
    if (!Math.floor(Math.random() * this.flightConsistency) && !this.maneuvering) {
      if (this.pos.x > game.canvasWidth / 2) {
        this.feint('left', Math.ceil(Math.random() * 3));
      } else {
        this.feint('right', Math.ceil(Math.random() * 3));
      }
    }
  }
};

Twinfighter.prototype.dodge = function (debris) {
  this.feint((debris.pos.x > this.pos.x ? 'left' : 'right'), 1 + Math.round(Math.random() * 12));
};
  ...
  for (j=0 ; j<game.debris.length ; j++) {
    ...
    if (!debris) {
         ...
         )
      {
        ...
        Math.abs(debris.pos.x - this.pos.x) < 64 &&
        Math.abs(debris.pos.y - this.pos.y) < 96 &&
        !this.coordinating
      ) {
        this.dodge(debris);
      }
